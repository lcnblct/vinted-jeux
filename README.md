...
- **État visible** : les échecs Vinted (partiels ou totaux : 500, timeouts) et les budgets dépassés restent verts (`[warn]`) : le bilan affiche les recherches réussies/échouées et le rattrapage se fait au run suivant (toutes les 5 min). Seule une livraison encore en attente produit un code de sortie non nul. Le watchdog (45 min sans succès) est le vrai signal de panne prolongée — 1 alerte/30 min au lieu d'1 e-mail/5 min. Le workflow sauvegarde l'état dans tous les cas.
- **Reprise** : `settings.scan_budget_seconds` vaut 200 par défaut (sous l'intervalle cron 5 min : total job ~260s, dans le timeout CI de 10 min). Le budget est vérifié entre les opérations ; un appel réseau déjà lancé peut finir après cette limite. La recherche interrompue est mémorisée dans `meta.scan_resume` et reprise au prochain passage. Les annonces déjà livrées ou rejetées pour cette recherche sont dédupliquées. La limite d'une page par recherche reste de 20 annonces par défaut : ce mécanisme ne garantit pas de retrouver une annonce sortie de cette page avant son traitement.
- **Connexion Vinted** : une session Vinted est réutilisée pour toute la watchlist afin d'éviter de redemander un cookie à chaque jeu. Une erreur réseau ou anti-bot transitoire (500, timeout, 429) est retentée jusqu'à cinq fois en backoff exponentiel (2s/4s/8s/16s/20s max), avec 1,5s de pacing entre recherches ; la session n'est renouvelée qu'après un 2e échec pour éviter le burst de cookies. Un échec, même total, reste visible dans le bilan sans faire rougir le run.
- **Contrôle de santé** : après un scan avec au moins une recherche réussie et sans livraison en attente, le bot enregistre `meta.last_successful_scan_at`. Un outage total ne le rafraîchit pas. Le workflow `Vinted Jeux — Watchdog` le vérifie toutes les 30 minutes et échoue s'il date de plus de 45 minutes. Les notifications dépendent de tes réglages GitHub Actions ; ce contrôle n'envoie pas de Telegram et dépend lui aussi de GitHub. Une panne générale de GitHub nécessite un contrôle extérieur indépendant. Le premier contrôle peut échouer tant qu'aucun scan de la nouvelle version n'a terminé.
